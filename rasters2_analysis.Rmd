...
Raster is a gird of numeric value, so it can be used and do operations as it was a vector. The difference is that it has a spacial component associated to it
...
##running operation to all the raster. The operation is applied to every singel cell in the raster.
...
##(values that are not = to 5 which means that they are not mozambique and values that are not NAs and set them to 0)
...
##deviding by the max value to get values form 0-1
...
##to get one plot from the list use [[1]] or the number you want tot plot. To access a list you use double square braquets
...
### a vector: 
###    from  to    class
##classification matrix saying 0 to 0.33 is a 1 low probability, etc..
m <- c(0.00, 0.33, 1,    
       0.33, 0.67, 2,    
       0.67, 1.00, 3)
...
```{r}
cellid_rast <- raster(eez_rast) %>% ##creats a blank raster with the same parametes as the eez_rast
...
shark_df <- data.frame(cell_id  = values(cellid_rast),
                       ...
                       eez      = values(eez_rast),
                       d_port   = values(port_dist_rast) / 1000) %>% ##cahnging from m to km
  ### now we can drop any cells outside the Mozambique EEZ (id = 5)
  filter(eez == 5)

### Now lets build our model. This calculations are easier to do in a df than a raster
...
``` {r}
## birnging the df back into raster to plot
x <- model_df %>%
...
ss_poly_sp <- raster::rasterToPolygons(ss_rast_hcaf, dissolve = FALSE) ## = T conects contiguos polygons
